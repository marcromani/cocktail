...
def whiten(X, num_components=None, center=True, rowvar=True):
    """Whitens the data in matrix X using PCA decomposition. The data corresponds to n
    samples of a p-dimensional random vector. The shape of the matrix can be either (n, p)
    if each row is considered to be a sample or (p, n) if each column is considered to be
    a sample. How to read the matrix entries is specified by the rowvar parameter. Before
    whitening, a dimensionality reduction step can be applied to the data to reduce the p
    dimensions of each sample to num_components dimensions. If num_components is None, the
    number of dimensions kept is the maximum possible (nº of non-zero eigenvalues). For
    example, if X is full rank (rank(X) = min(n, p)), then num_components = p if p < n,
    and num_components = n-1 if p >= n.
...
def non_negative_ICA(X, num_sources, lr=0.03, max_iter=5000, tol=1e-8, rowvar=True):
    """Computes the non-negative independent components of the linear generative model
    x = A * s, where x is a p-dimensional observable random vector and s is the latent
    random vector of length num_sources, whose components are statistically independent
    and non-negative. The matrix X is assumed to hold n samples of x, stacked in rows
    (shape(X) = (n, p)) or columns (shape(X) = (p, n)), which can be specified by the
    rowvar parameter. In practice, if shape(X) = (p, n) (resp. shape(X) = (n, p)) this
    function solves X = A * S (resp. X = S.T * A.T) both for S and A, where A is the
    so-called mixing matrix, with shape (p, num_sources), and S is a (num_sources, n)
    matrix which contains n samples of the latent source vector, stacked in columns.
